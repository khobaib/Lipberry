	...
	public void setmemberlist() {
		...
		if (commentslist.getCommentslist().size() > 0) {
			...
			adapter1 = new CustomAdapterForComment(getActivity(), commentslist.getCommentslist(), Constants.baseurl
					+ "article/commentlist/" + articledetails.getId(), 0, this, null, null);
...
//	   ImageLoader.getInstance().init(config);
	...
	public void onResume() {
		...
		if (articledetails.getCategory_title() != null) {
			...
			if (articledetails.getCategory().equals("2")) {
				...
				// }
				// else{
				if(articledetails.getCategory_title().contains(getActivity().getString(R.string.txt_cat2_shpx))){
					((HomeActivity) getActivity()).welcome_title.setText(getResources().getString(R.string.txt_cat2_shpx));
	...
	public void showCustomDialog() {
		/*
		 * AlertDialog.Builder builder; final AlertDialog alertDialog;
		 * 
		 * Context mContext = getActivity().getApplicationContext();
		 * LayoutInflater inflater = (LayoutInflater)
		 * mContext.getSystemService(getActivity().LAYOUT_INFLATER_SERVICE);
		 * View layout = inflater.inflate(R.layout.custom_dilog, null); builder
		 * = new AlertDialog.Builder(mContext); builder.setView(layout);
		 * alertDialog = builder.create(); et_comment = (EditText)
		 * layout.findViewById(R.id.et_comment); Button btn_cancel = (Button)
		 * layout.findViewById(R.id.btn_cancel); Button bt_ok = (Button)
		 * layout.findViewById(R.id.bt_ok); btn_cancel.setOnClickListener(new
		 * OnClickListener() {
		 * 
		 ...
		 * comments=et_comment.getText().toString(); if(comments.equals("")){
		 * Toast.makeText(getActivity(),
		 * getActivity().getResources().getString(R.string.Toast_enter_text),
		 * Toast.LENGTH_SHORT).show(); } else{ alertDialog.cancel();
		 * commentstext=comments; sendposttoserver(); } } });
		 */
